...
class Tagger:
	...
	def viterbi(self, sen):
		...
		for z in range(len(self.states)):
			g = ('*','*',self.states[z])
			a = self.backoff_dict.get(g, self.katz_backoff(self.n,g))
		...
		for i in range(1, len(sen)-3):
			for m in range(len(self.states)):
				...
				for n in range(len(path_prob[i-1])):
					...
					pro = path_prob[i-1][n][0] * self.backoff_dict.get(gram, self.katz_backoff(self.n,gram)) * self.prob_word(self.states[m], sen[2+i].lower())
					if pro > 0:
						p.append(pro)
					else:
						continue
